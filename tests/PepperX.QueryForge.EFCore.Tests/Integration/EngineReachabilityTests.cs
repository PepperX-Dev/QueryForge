...
public sealed class EngineReachabilityTests
{
    ...
    {
        Skip.IfNot(
            EfCoreEngines.EnginesAreRequired,
            $"Reachability is advisory here; set {EfCoreEngines.RequireVariable}=1 to enforce it.");

        var missing = EfCoreEngines.ConfiguredButUnreachable();
